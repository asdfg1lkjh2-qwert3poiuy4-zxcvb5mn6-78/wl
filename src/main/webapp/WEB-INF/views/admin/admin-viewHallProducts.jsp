...
<body class="theme-blush">
	...
	function fetchAllProducts(){
		$.ajax({
			type : "GET",
			url : "admin-fetchAllHallsBySellerId",
			data : "",
			processData : false,
			...
			success : function(data) {
				if(data.status){
					$("#productPageDiv").html("");
					var abc ="";
					for(var i = 0; i< data.listAllHalls.length; i++){
						var url = "<img src=\"getImage?id="+ data.listAllHalls[i].dpUrl+ "\" class=\"img-responsive\"/>"
						...
						+url
						+"<div class=\"caption\">"
						+"<h3>"+data.listAllHalls[i].name+"</h3>"
						+"<p>"+price+"</p>" 
						+"<p align=\"center\">"
						+"<a href=\"admin-addEditHall?id="+data.listAllHalls[i].id+"\"class=\"btn btn-block gradient-right waves-effect\">View Product</a>"
						+"</p>"
						+"</div>"
					    +"</div>"
				        +"</div>";
